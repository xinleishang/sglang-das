...
def _jit_sparse_module(
    ...
    # (False, False) production instantiation keeps the regular path unchanged.
...
def copy_cache_planned_mla(
    *,
    ...
    num_blocks=num_reqs (one block per request, padded to the graph capture size
    like load_cache_to_device_buffer's grid) so this scales with the batch.
    """
    assert miss_src.dtype == torch.int64 and miss_dst.dtype == torch.int32
    assert miss_count.dtype == torch.int32
    block_size = resolve_hisparse_block_size(block_size)
