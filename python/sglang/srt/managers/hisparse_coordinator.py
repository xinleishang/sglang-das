...
from sglang.srt.configs.model_config import is_deepseek_nsa, nsa_layer_skips_topk
...
def resolve_shared_index_layers(
    *,
    ...
    Mirrors attention's skip_topk derivation (index_topk_pattern /
    ...
    """
    if not is_deepseek_nsa(hf_text_config):
        return None
    ...
    if cli_factor > 1:
        ...
        pattern = [nsa_layer_skips_topk(hf_text_config, i) for i in range(num_layers)]
    if not any(pattern):
        return None
...
class HiSparseCoordinator:
    ...
    def _init_shared_index_prefetch(
        ...
        """Set up plan-then-IO prefetch for shared-index (IndexShare) models."""
        ...
        # The anchor records its miss plan once; skip layers replay it through
        # copy_cache_planned_kernel, launched with one block per request
        # (num_blocks=num_reqs) so its parallelism scales with the batch. One
    ...
    def _run_copy_only_kernel(self, num_reqs: int, skip_layer: int) -> None:
        ...
        copy_cache_planned_mla(
            ...
            host_cache_ptrs=getattr(self.mem_pool_host, "data_ptrs", None),
            host_cache_ptr_index=skip_layer,
